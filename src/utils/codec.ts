...
export function decodePoolV3(encoding: bigint): string {
  ...
  switch (poolType) {
    ...
    case PoolTypeV3.ERC4626: {
      ...
      const token = Object.entries(EthereumTokens).find(([symbol, metadata]) => getAddress(metadata.address) === pool);
      if (token) {
...
export function encodeChainlinkPriceFeed(feed: string, scale: bigint, heartbeat: number): string {
  return "0x" + ((BigInt(feed) << 96n) | (scale << 32n) | BigInt(heartbeat)).toString(16).padStart(64, "0");
}
