...
contract LongPositionEmergencyCloseFacet is MorphoFlashLoanFacetBase, IPositionOperateFacet {
  ...
  /// @param data 传递给`onCloseOrRemoveLongPositionFlashLoan`的钩子数据
  ...
  /// @notice `closeOrRemoveLongPositionFlashLoanWithUSDC`的回调钩子
  /// @param pool The address of fx position pool.
  /// @param pool fx仓位池地址
  /// @param position The index of position.
  /// @param position 仓位索引
  ...
  /// @param data 传递给`onCloseOrRemoveLongPositionFlashLoanWithUSDC`的钩子数据
  ...
  /// @dev 内部函数：将信用票据赎回为fxUSD并兑换为抵押品代币
  /// @param pool The address of fx position pool.
  /// @param pool fx仓位池地址
  /// @param data Hook data passing to `onCloseOrRemoveLongPositionFlashLoan` or `onCloseOrRemoveLongPositionFlashLoanWithUSDC`.
  /// @param data 传递给`onCloseOrRemoveLongPositionFlashLoan`或`onCloseOrRemoveLongPositionFlashLoanWithUSDC`的钩子数据
  ...
  /// @dev Internal function to do swap with converter. 内部函数：使用转换器执行代币兑换
  /// @param token The address of input token. 输入代币地址
  /// @param amountIn The amount of input token. 输入代币数量
  /// @param minOut The minimum amount of output tokens should receive. 应收到的最小输出代币数量
  /// @param encoding The encoding for swap routes. 兑换路由的编码
  /// @param routes The swap routes to `MultiPathConverter`. 传递给`MultiPathConverter`的兑换路由
  /// @return amountOut The amount of output tokens received. 收到的输出代币数量
