...
/// @title PoolManager - 池管理器合约
/// @notice 管理多头池的核心操作，包括开仓、平仓、赎回、再平衡和清算
/// @dev 继承自 ProtocolFees、FlashLoans 和 AssetManagement
///
/// ==================== 池管理器概述 ====================
///
/// 1. 仓位操作 (operate)
...
contract PoolManager is ProtocolFees, FlashLoans, AssetManagement, ILongPoolManager {
  ...
  /// @dev 池信息结构体
  /// @param collateralData 抵押品数据
  ///   ```text
  ///   * 字段                      位数    索引        说明
  ///   * collateral capacity       85      0           最大允许的抵押品数量
  ///   * collateral balance        85      85          已存入的抵押品数量
  ///   * raw collateral balance    86      170         池中管理的原始抵押品数量（不含汇率）
  ///   ```
  /// @param debtData 债务数据
  ///   ```text
  ///   * 字段             位数    索引        说明
  ///   * debt capacity     96      0           最大允许的债务数量
  ///   * debt balance      96      96          已借出的债务数量
  ///   * reserved          64      192         保留数据
  ///   ```
  struct PoolStruct {
  ...
  /// @dev 代币汇率信息结构体
  /// @param scalar 代币缩放因子（转换为 18 位精度）
  /// @param rateProvider 汇率提供者地址
  struct TokenRate {
    uint96 scalar;
  ...
  /// @dev 清算或再平衡的内存变量
  /// @param stablePrice 稳定币的 USD 价格（带缩放）
  /// @param scalingFactor 抵押品代币的缩放因子
  /// @param collateralToken 抵押品代币地址
  /// @param rawColls 清算或再平衡的原始抵押品数量（含奖励）
  /// @param bonusRawColls 作为奖励的原始抵押品数量
  /// @param rawDebts 清算或再平衡的原始债务数量
  ...
  struct OperationMemoryVar {
    ...
    uint256 poolSupplyFeeRatio;    // 池供应费率
    uint256 poolWithdrawFeeRatio;  // 池提款费率
    uint256 poolBorrowFeeRatio;    // 池借款费率
    uint256 poolRepayFeeRatio;     // 池还款费率
    address referral;              // 推荐人地址
    uint256 referralSupplyFeeRatio;   // 推荐人供应费率
    uint256 referralWithdrawFeeRatio; // 推荐人提款费率
    uint256 referralBorrowFeeRatio;   // 推荐人借款费率
    uint256 referralRepayFeeRatio;    // 推荐人还款费率
  }
  ...
  /// @dev 验证调用者是否为 FxUSDSave 或允许无许可操作
  modifier onlyFxUSDSave() {
    if (_msgSender() != fxBASE) {
      // 当 fxBASE 中的 fxUSD/USDC 不足时，允许无许可再平衡或清算
      // 使用总供应量来估计 fxBASE 中的 fxUSD/USDC 数量
  ...
  /// @param _counterparty 对手方（空头池管理器）地址
  /// @param _configuration 配置合约地址
  /// @param _whitelist 智能钱包白名单地址
  ...
  /// @notice 初始化函数（代理模式）
  /// @param admin 管理员地址
  /// @param _expenseRatio 费用比率
  /// @param _harvesterRatio 收割者比率
  /// @param _flashLoanFeeRatio 闪电贷费率
  /// @param _treasury 国库地址
  /// @param _revenuePool 收益池地址
  ...
  */
  ...
  /// @notice 返回池信息
  /// @param pool 要查询的池地址
  /// @return collateralCapacity 最大允许的抵押品数量
  /// @return collateralBalance 已存入的抵押品数量
  /// @return rawCollateral 已存入的原始抵押品数量
  /// @return debtCapacity 最大允许的债务数量
  /// @return debtBalance 已借出的债务数量
